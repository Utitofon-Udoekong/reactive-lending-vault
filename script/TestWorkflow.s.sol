...
contract TestWorkflow is Script {
    function run() external {
        ...
        console.log(
            "  --sig 'updatePoolARates()' - Update Pool A rates to trigger rebalance"
        );
        console.log("  --sig 'updatePoolBRates()' - Update Pool B rates");
        console.log(
            "  --sig 'checkStatus()'     - Check current allocation and rates"
        );
    }
    ...
    function depositToVault() external {
        ...
        LendingVault vault = LendingVault(payable(vaultAddr));
